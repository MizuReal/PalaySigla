// Per-step validation for the posting wizard — a direct port of
// website/src/utils/listingValidation.ts. The photo input is the prepared
// (compressed) image rather than a web Blob; every message and bound stays
// identical to the website so the flows read the same on both platforms.
...
interface ListingPhotoInput {
  image: PreparedImage | null | undefined
}
...
function validatePhoto({ image }: Partial<ListingPhotoInput>): ListingStepErrors {
  const errors: ListingStepErrors = {}
  if (!image) {
