...
describe('validateListingStep step 1 (details)', () => {
  ...
  it('trims the title before checking its length', () => {
    expect(
      validateListingStep(1, { ...VALID_DETAILS, title: '  ab  ' }).title
    ).toBeDefined()
    expect(
      validateListingStep(1, { ...VALID_DETAILS, title: '  abc  ' }).title
    ).toBeUndefined()
  })
  ...
  it('rejects unknown units and categories', () => {
    expect(validateListingStep(1, { ...VALID_DETAILS, unit: 'ton' }).unit).toBe(
      'Choose a unit.'
    )
    expect(
      validateListingStep(1, { ...VALID_DETAILS, category: 'feed' }).category
    ).toBe('Choose a category.')
  })

  it('allows an empty quantity but rejects invalid values', () => {
    expect(
      validateListingStep(1, { ...VALID_DETAILS, quantity: '' }).quantity
    ).toBeUndefined()
...
describe('validateListingStep step 2 (photo)', () => {
  it('requires a prepared image', () => {
    expect(validateListingStep(2, {})).toEqual({
      photo: 'Add a photo of your listing.',
    })
    expect(validateListingStep(2, { image: null })).toEqual({
      photo: 'Add a photo of your listing.',
    })
    expect(validateListingStep(2, { image: IMAGE })).toEqual({})
  })
})
...
describe('validateListingStep step 3 (location)', () => {
  it('requires both coordinates and a non-blank label', () => {
    ...
    expect(
      validateListingStep(3, { lat: null, lng: 121, locationLabel: 'Manila' })
    ).toEqual(expected)
    expect(
      validateListingStep(3, { lat: 14, lng: null, locationLabel: 'Manila' })
    ).toEqual(expected)
    expect(validateListingStep(3, { lat: 14, lng: 121, locationLabel: '   ' })).toEqual(
      expected
    )
    expect(
      validateListingStep(3, { lat: 14, lng: 121, locationLabel: 'Manila' })
    ).toEqual({})
  })
