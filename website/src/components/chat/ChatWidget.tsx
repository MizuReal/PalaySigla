...
function readStoredMessages(storageKey: string): ChatTurn[] {
  try {
    ...
    return parsed
      ...
      .map((turn) => ({
        role: turn.role,
        content: stripLegacyMarkdown(turn.content),
      }))
  } catch {
...
function writeStoredMessages(storageKey: string, messages: ChatTurn[]): void {
  try {
...
function clearStoredMessages(storageKey: string): void {
  try {
...
function WelcomeBubble({ onPick }: { onPick: (question: string) => void }) {
  return (
  ...
  const submitMessage = useCallback(
    async (content: string, baseMessages: ChatTurn[]) => {
      ...
      try {
        ...
        const completed = [...nextMessages, assistantTurn].slice(-MAX_HISTORY_MESSAGES)
