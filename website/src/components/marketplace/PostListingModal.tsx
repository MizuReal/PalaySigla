...
const FIELD_ID_MAP: Record<string, string> = Object.freeze({
  title: 'title',
...
function PostListingModal({ onClose, onPosted }: PostListingModalProps) {
  ...
  const clearFieldError = (field: string) => {
    setErrors((current) => {
      ...
      const next = { ...current }
      delete next[field as keyof ListingStepErrors]
      return next
    })
  }
  ...
  const handleSubmit = async () => {
    ...
    if (!imageFile || !position || !isListingUnit(unit) || !isListingCategory(category)) {
      return
    }
    setServerError('')
    try {
