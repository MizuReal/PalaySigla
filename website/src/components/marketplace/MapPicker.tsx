...
interface ClickToPositionProps {
  onPositionChange: (position: MapPosition) => void
}

function ClickToPosition({ onPositionChange }: ClickToPositionProps) {
  useMapEvents({
    click: (event) => {
...
function MapPicker({ position, onPositionChange, onLocationLabel }: MapPickerProps) {
  ...
  const handleSearchSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setSearchError(
        err instanceof Error ? err.message : 'Location service returned an error.'
      )
      setSearchResults([])
    } finally {
