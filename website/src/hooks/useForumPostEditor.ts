      ...
      try {
        if (!post) {
          ...
          try {
            ...
            // roll back the uploads and the photo-less post, mirroring the
            // marketplace's create-then-upload flow
        ...
        for (const file of newImages) {
          await uploadForumImage({ file, postId: post.id, userId: user.id, position })
          position += 1
        }
        if (
